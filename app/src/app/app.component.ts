...
export class AppComponent implements OnInit {
  ...
  constructor(private alltomp3: Alltomp3Service, private db: DatabaseService, public contextMenu: ContextMenuService) {
    ...
    setInterval(() => {
      this.changePlaceholder.apply(this);
    }, 5000);
  }
  ...
  private changePlaceholder() {
    ...
    setTimeout(() => {
      (<HTMLInputElement>document.getElementById('input-main')).placeholder =
        this.helpProposals[this.currentProposal++ % this.helpProposals.length];
      if (this.suggestions.songs || this.suggestions.albums) {
  ...
  public execute(event: any) {
    ...
    const type = this.lastResult.type;
    if (type === 'single-url') {
      ...
      const songsl = this.suggestions.songs.length;
      let suggestion = this.suggestions.songs[this.activeSuggestion];
      let stype = 'track';
      if (this.activeSuggestion >= songsl) {
        suggestion = this.suggestions.albums[this.activeSuggestion - songsl];
        stype = 'album';
      } else {
